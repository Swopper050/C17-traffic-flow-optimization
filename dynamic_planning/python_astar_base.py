...
This code is taken directly from the python-astar package, more specifically from:
    https://github.com/jrialland/python-astar/blob/master/astar/__init__.py

The code is modified by us such that time is taken into account when constructing a path.
"""
...
class AStar:
    ...
    class SearchNode:
        ...
        def __init__(self, data, *, timestep_at_node, gscore=Infinite, fscore=Infinite):
            self.data = data
            self.gscore = gscore
            self.fscore = fscore
            self.timestep_at_node = timestep_at_node
            self.closed = False
    ...
    def astar(self, start, goal, *, start_time, reversePath=False):
