...
def geo_converter(lon, lat):
    transformer = Transformer.from_crs("epsg:4326", "epsg:8826")
    x1, y1 = transformer.transform(lon, lat)
    x2, y2 = transformer.transform(min_lon, min_lat)
    return (x1 - x2), (y1 - y2)
...
def draw(nodes, CityFlowNet, html):
    ...
    count = 0
    for i, (nodeID, way_list) in enumerate(cross.items()):
        print(f"At {i}/{len(cross.items())}", end="\r")
        count += 1
        if count % 100 == 0:
            print(count)
        if len(way_list) > 1:
